...
const About = (function () {
  ...
  const NUM_OF_PROFILES = 7;
  const NUM_OF_GROUPS = 6;
  const NUM_OF_TITLES = 1;
  ...
  animateProfile = function (profileNum, percentage, isAnimateIn) {
    ...
    if (isAnimateIn) {
      ...
      if (percentage < .09) {
        $(div.children[0].children[0]).css({ 'width': `85%`, 'opacity': `.1`, 'filter': `grayscale(100%)` })
        $(div.children[1].children[0]).css({ 'opacity': `0`, 'filter': `grayscale(100%)` })
        $(div.children[1].children[1]).css({ 'opacity': `0` })
        $(div.children[2]).css({ 'transform': `rotate(29deg)`, 'left': `30%` })
      }
      else {
        $(div.children[0].children[0]).css({ 'width': `${85 + (15 * percentage)}%`, 'opacity': `${.1 + (.9 * percentage)}`, 'filter': `grayscale(${100 - (100 * percentage)}%)` })
        ...
        $(div.children[2]).css({ 'transform': `rotate(${29 - (29 * percentage)}deg)`, 'left': `${30 - (30 * percentage)}%` })
      }

    }
    else {
      if (percentage !== 1) {
        $(div.children[0].children[0]).css({ 'width': `${100 - (15 * percentage)}%`, 'opacity': `${1 - (.9 * percentage)}`, 'filter': `grayscale(${0 + (100 * percentage)}%)` })
      }
      ...
      $(div.children[2]).css({ 'transform': `rotate(${(29 * percentage)}deg)`, 'left': `${(30 * percentage)}%` })
    }
  }
